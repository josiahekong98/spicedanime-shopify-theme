...
document.addEventListener("DOMContentLoaded", () => {
  let loadMoreBtn = document.querySelector(".js-btn-load-more");
  const productGrid = document.getElementById("product-grid");

  if (!loadMoreBtn || !productGrid) return;

  let loading = false;
  let maxScrollItems = 50; // limit before switching to pagination

  function autoLoad() {
    if (loading) return;

    const cardCount = productGrid.querySelectorAll(".card-product, .product-card").length;
    
    // Stop infinite scroll once threshold reached
    if (cardCount >= maxScrollItems) {
    ...
    if (rect.top < window.innerHeight + 900) {
      loadMoreBtn.click(); // trigger load
    }
  }
